...
df_est <- lapply(filename, function(x) {
  ...
  df_m <- df0 %>% 
    ...
    mutate(prob_negative = 1 - prob_positive,
           Parameter = case_when(parameter == "a[1]" ~ "$\\beta_1$",
                                 parameter == "a[2]" ~ "$\\beta_2$",
                                 parameter == "a[3]" ~ "$\\beta_3$",
                                 parameter == "a[4]" ~ "$\\beta_4$",
                                 parameter == "b[1]" ~ "$\\gamma_0$",
                                 parameter == "b[2]" ~ "$\\gamma_1$",
                                 parameter == "b[3]" ~ "$\\gamma_2$"),
