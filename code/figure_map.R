...
## hokkaido shape
albers_sf_hkd <- st_read(here::here("data_gis/albers_hkd_shape.gpkg"),
                         quiet = TRUE) %>% 
  st_set_crs(wkt_jgd_albers)

## stream network
albers_sf_channel <- st_read(here::here("data_gis/albers_channel_hkd.gpkg"),
                             quiet = TRUE) %>% 
  st_set_crs(wkt_jgd_albers)
...
## watershed
albers_sf_wsd <- st_read(here::here("data_gis/albers_watershed_hkd_final.gpkg"),
                         quiet = TRUE) %>% 
  st_set_crs(wkt_jgd_albers)
...
albers_sf_point <- st_read(here::here("data_gis/epsg4326_point_snap_prtwsd_edit.gpkg"),
                           quiet = TRUE) %>% 
