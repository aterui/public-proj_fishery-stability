...
df_mcmc <- lapply(c("summary_joint", "summary_reg", "summary_multi_ricker"),
       function(x) {
         list.files(path = here::here("output"),
           ...
           dplyr::select(n_total_mcmc,
                         ...
                         n_burn) %>% 
           distinct() %>% 
           mutate(model = str_extract(x, "joint|reg|ricker"))
       }) %>% 
  bind_rows()
...
**Priors.** Vague priors were assigned to most parameters: $\text{Normal(0, 10)}$ for regression coefficients ($\gamma$ and $\gamma'$) and means ($\theta_{\beta}$ and $\pmb{\theta_{\xi}}$), $\text{Half-student's}~t(0, 2.5, 6)$ for SDs ($\sigma_{\beta}$, $\sigma_{\text{obs},s}$, $\sigma_{\text{state},s}$, $\sigma_{\text{obs},i}$, $\sigma_{\text{state},i}$ $\sigma_{\lambda}$, $\sigma_{\gamma}$, and $\sigma_{\epsilon,i}$), $\text{scaled-Inv-Wishart}(\pmb{\phi})$ ($\pmb{\phi}$ is a vector of prior scales $\pmb{\phi}=\{2.5,...,2.5\}$) with the degrees of freedom = 2 for variance-covariance matrices ($\pmb{\Omega_{\xi}}$, $\pmb{\Omega_y}$, and $\pmb{\Omega_{\varepsilon}}$), and $\text{Normal}(0, 1)$ for latent factors ($\zeta_{t,d}$). We used multiplicative gamma priors with parameters $\nu = 3$, $a_1=2$, and $a_2=3$ (see Durante [@duranteNoteMultiplicativeGamma2017] for interpretation) for latent factors ($\delta_{d,i}$) following Ovaskainen *et al.* [@ovaskainenHowAreSpecies2017]. We assigned a weakly informative priors to initial fish densities $\text{ln}~n_{i,t}$ ($t = 1, 2, 3$) using $\text{Student's}~t(-2, 2, 6)$ to enhance model convergence.
