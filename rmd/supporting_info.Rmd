...
```{=tex}
\begin{align}
N_{i,t+1} = N_{i,t}~\exp\left[r_i \left(1-\frac{N_{i,t} + \sum_{j=1}^S \alpha_{ij}N_{j,t}}{K_i} \right) \right]\exp(\varepsilon_{i,t})
\end{align}
```
...
```{=tex}
\begin{align}
N_{i,t+1} = (N_{i,t}+\phi_i R_t)~\exp \left[r_i \left\{1-\frac{N_{i,t}+\alpha_{i1} (N_{1,t} + R_t) + \sum_{j=2}^S \alpha_{ij}N_{j,t}}{K_i} \right\} \right]\exp(\varepsilon_{i,t})
\end{align}
```
...
```{=tex}
\begin{align}
N_{i,t+1} = (N_{i,t}+\phi_i R_t) ~ \exp \left[r_i \left(1-\frac{N_{i,t}+\alpha_{i1}R_t + \sum_{j=1}^S \alpha_{ij}N_{j,t}}{K_i} \right) \right]\exp(\varepsilon_{i,t})
\end{align}
```
...
\ln \pmb{y_s} &\sim \text{MN}(\pmb{\theta_{y,s}}, \pmb{\Omega_y}) &&\text{for mean and SD}
\end{aligned}
\right.
 (\#eq:reg)
\end{align}
```
In the poisson model for species richness, $\exp(\varepsilon_{\lambda,s})$ is the error term accounting for overdispersion ($\varepsilon_{\lambda,s} \sim \text{Normal}(0, \sigma_{\lambda}^2)$). In the multi-variate normal model, $\pmb{y_s}$ represents a vector of mean and SD at site $s$, which is modeled as a random variable drawn from a multi-variate normal distribution with a variance-covariance matrix $\pmb{\Omega_y}$. The expected means $\lambda_{y,s}$ and $\pmb{\theta_s}$ were related to linear predictors as follows:
...
\end{align}
```
$\gamma_{0,w(s)}$ is the watershed-specific intercept ($w(s)$ refers to site $s$ nested within watershed $w$) and $\gamma_k (k > 0)$ are the regression coefficients of effective release of masu salmon $R'_s$ and other site-level predictors $x_k$. $R'_s$ is the product of the average yearly release at watershed $w$ ($R_w$; fry + juvenile + smolt, averaged for 1999 -- 2019) and the site-specific weight factor $\eta_{w,s'}$ ($s'=1,...,S_w + 1$, where $S_w$ is the number of sites within watershed $w$):
...
As for the effects of environmental variables on the CV, we did not explicitly develop a regression model. Instead, we derived their effects from the multi-variate normal model of mean and SD since the CV is a additive function of mean and SD in a log scale:
...
`r paste(knitr::knit(text = table_stvy_chunk), collapse = '\n')`
