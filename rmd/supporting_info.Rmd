...
**Time-series data.** We assembled time-series data of fish abundance across `r n_distinct(d0$site_id)` sites within `r n_distinct(d0$river)` protected watersheds of Hokkaido Island, Japan. At these sites, the long-term monitoring program began in 1971 but effective sampling methods (a combination of electrofishing and cast net sampling) were implemented since 1999. Most data were collected in summer with irregular interannual intervals (1- to 3-year interval for most cases). We confined our analysis to the sites that meets the following criteria: (i) the observation span (from the first to the last year of observation) exceeds 10 years, (ii) the number of observation years exceeds five years, and (iii) masu salmon are observed at least twice during the observation period. As a result, we used time-series data of fish abundance at `r n_distinct(df_fish$site_id)` sites within `r n_distinct(df_fish$river)` watersheds during a period of 1999-2019. **Table S5** summarizes observed species in these watersheds.
...
\end{align}
```
$n_{s,t}$ is the "true" fish density excluding fish released in the spring, $Fry_{w(s),t}$ the number of salmon fry released in spring in watershed $w$ within which site $s$ is located, and $\beta_s$ the site-specific effect of released salmon fry on the observed fish density. The parameter $\beta_s$ was drawn from a normal distribution with the hyper-mean $\mu_{\beta}$ and hyper-variance $\sigma_{\beta}^2$. The parameter $\epsilon_{s,t}^{obs}$ is the normally-distributed error term ($\epsilon_{s,t}^{obs} \sim Normal(0, \sigma_{obs,s}^2)$) that accounts for site- and year-specific observation errors. When modeling the unenhanced species group, $\psi$ equals zero (otherwise $\psi = 1$) so the model excludes the term $\beta_s~Fry_{w(s),t}$.
...
\end{align}
```
$\delta_{0}$ is the watershed-specific intercept and $\delta_k (k > 0)$ are the regression coefficients. The watershed-level predictors $z_{k,w}$ include the number of masu salmon released (average of 1999-2019) and ocean productivity (Chlorophyll a). The parameter $\sigma_{\gamma}$ accounts for random variation among watersheds that cannot be captured by the watershed-level predictors.
...
Median estimates of the Bayesian state space model. Site specific parameters were excluded due to the large number of parameters. Numbers in parenthesis indicate 95% credible intervals.
