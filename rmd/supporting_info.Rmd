...
for(i in seq_len(nrow(df_param))) {
  ...
  knit_chunk <- paste0("## Theoretical prediction (",
                       "$r_1$ = `r df_param$r1[",i,"]`, ",
                       "$K$ = `r df_param$k[",i,"]`) \n",
                       "\n```{r fig-sim", i,", fig.width = 10, fig.height = 7, fig.cap = cap[", i, "]}
                       \n\n
                       list_g_theory[[",i,"]]
                       \n\n
                       ```\n",
...
```{r fig-int, fig.width=11, fig.height=11, fig.cap="Estimates of competition coefficients $\\alpha_{ij}$ in a multi-species Ricker model. Panels distinguish sites, and values in each cell indicate median estimates of $\\alpha_{ij}$ for a given pair of species. Cells are colored in proportion to $\\alpha_{ij}$."}
