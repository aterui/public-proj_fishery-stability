...
In our simulation, we considered `r nrow(param_set)` simulation scenarios with different values of the following ecological parameters: intrinsic growth of the enhanced species $r_1$ (`r unique(sim_result$r1)`), mean strength of interspecific competition $\bar{\alpha}$ (`r unique(sim_result$alpha)`), carrying capacity $K$ (`r unique(sim_result$k)`), relative fitness of captive-bred individuals $f_R$ (`r unique(sim_result$phi)`), and degree of environmental stochasticity $\sigma_{\epsilon}$ (`r unique(sim_result$sd_env)`). We chose these parameters because our sensitivity analysis revealed that these parameters have the potential to alter the relationship between community dynamics and stock enhancement (**Supporting Information**). Under each scenario with a unique parameter combination, we ran `r n_sim` time steps of `r max(sim_result$n_rep)` independent communities (i.e., simulation replicates) with varying numbers of released individuals $R$ ($0-500$). The last `r unique(sim_result$n_timestep)` time steps were used to obtain the following summary statistics of total community density $\sum_i^SN_i$: the number of species persist (defined as $N_i > 0.01$ at $t=1000$), the coefficient of variation (CV), the temporal mean, and the temporal SD. In addition, temporal means and SDs were calculated separately for the enhanced species $N_1$ and unenhanced species $\sum_{i,i \ne 1}^S N_i$ to infer mechanisms that underlie the community-level response.
...
In this study, we modified a multispecies Ricker model to include selective stock enhancement of a constituent species (species 1). In the basic model formula without stock enhancement, population density of species $i$ at time $t+1$, $N_{i,t+1}$, is modeled as:
...
where $r_i$ is the intrinsic growth rate, $\alpha_{ij}$ the competition coefficient between species $i$ and $j$, $K_i$ the carrying capacity, and $\epsilon_{i,t}$ the species responses to stochastic environmental fluctuations that obey an normal distribution $\epsilon_{i,t} \sim Normal(0, \sigma_{\epsilon}^2)$. We modified this model formula to consider the effects of stock enhancement on reproduction and competition as follows:
...
$R_t$ is the number of released individuals, and the parameter $\phi_i$ controls the relative fitness of captive-bred individuals as follows:
...
We developed the following linear regression models taking either CV, species richness, mean, or SD as a response variable $y_s$ with a normal or Poisson distribution.

$$
\begin{aligned}
\left\{
\begin{array}{}
ln~y_s &\sim Normal(\mu_y, \sigma_y^2)~&~\text{for CV, mean, and SD}\\
...
$$
\begin{aligned}
\mu_{y,s} &= \gamma_{0,w(s)} + \sum_k \gamma_kx_{k,s} &\text{(10)}
\end{aligned}
$$
...
We fitted the models to the data using JAGS version 4.1.0 through *runjags* package in R. We assigned weakly informative priors for parameters, which were summarized in XXX. Data manipulation and analysis were performed in R version 4.1.0.
