...
To gain better insights into the driving mechanisms, we fitted a multi-species Ricker model to a subset of our time-series data at nine sites with few missing observations and no intentional release (**Methods**); thus, the parameter estimates of this model is directly comparable to those used in the theoretical Ricker model. The estimated competition coefficients $\alpha_{ij}$ ranged `r round(min(df_pd$alpha_prime), 2)` -- `r round(max(df_pd$alpha_prime), 2)` with the median of `r round(median(df_pd$alpha_prime), 2)` (Figure 3), suggesting that competition is a likely mechanism as our theory assumes. Our statistical inference is supported by previous observational studies. For example, interference competition between masu salmon and white-spotted charr is well studied. Their diets are innately overlapped (e.g., drift terrestrial insects), but they choose different prey items when they co-occur [@miyasakaFoodHabitDivergence2003]. Exploitative competition between masu salmon and benthic fish (e.g., sculpins) is also likely because subordinate masu salmon in the dominance hierarchy consumes benthic macroinvertebrates [@satoStagespecificFunctionalResponses2014], which are dominant prey items for benthic fish species. Importantly, hatchery masu salmon may intensify intra- and interspecific competition in the wild [@hasegawaWildMasuSalmon2018; @sahashiEvaluationContributionHatchery2015; @hasegawaPulsedSuppliesSmall2022]. Hatchery salmons are larger and more aggressive than wild individuals, increasing the likelihood of intense intra- and interspecific competition [@weberInteractionsHatcheryWild2003]. Indeed, field and experimental studies confirmed that hatchery masu salmon competed with wild masu salmon and other stream fishes during their freshwater life stage [@hasegawaWildMasuSalmon2018; @sahashiEvaluationContributionHatchery2015; @hasegawaPulsedSuppliesSmall2022].
...
Importantly, socioeconomic factors control the number of releases (e.g., budget allocation) regardless of the current condition of recipient communities. As such, released fish are probably "excessive" and may cause resource competition that would otherwise not exist.
