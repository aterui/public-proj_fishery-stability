...
In our simulation, we used `r nrow(param_set)` simulation scenarios with varied values of the following ecological parameters: mean strength of interspecific competition $\bar{\alpha}$, intrinsic growth of the enhanced species $r_1$, relative fitness of captive-bred individuals $f_R$, and degree of environmental stochasticity $\sigma_{\epsilon}$. Under each scenario (i.e., parameter combination), we ran 1400 time steps of 1000 independent communities (i.e., simulation replicates) with varying $R$. The last 1000 time steps were used to obtain the coefficient of variation, temporal mean, and temporal standard deviation of total community density ($\sum_i^SN_i$). Temporal means and standard deviations were calculated separately for the enhanced species $N_1$ and the unenhanced species $\sum_{i,i \ne 1}^S N_i$ to infer mechanisms that underlie community-level responses.
